  ...
  useEffect(()=>{
    ...
    const onKey=(e:KeyboardEvent)=>{
      ...
      if(e.key==='Tab'){
        ...
        if(!root) return
        const focusables = Array.from(root.querySelectorAll<HTMLElement>('a,button,[tabindex]:not([tabindex="-1"])'))
    ...
    return ()=>{ window.removeEventListener('keydown', onKey); document.body.style.overflow=prevOverflow; clearTimeout(t) }
  },[open])

  const NavLink = ({href,label,slug}:{href:string,label:string,slug:string}) => (
    <Link href={href} onClick={()=>setOpen(false)} className={clsx('block py-2 text-lg', isActive(slug) && 'font-semibold underline')} aria-current={isActive(slug)?'page':undefined}>
      {label}
    </Link>
  )
    ...
    <div className="md:hidden">
      ...
      {open && (
        <Portal>
          ...
          <div role="dialog" aria-modal="true" className="mobile-panel slide-in" ref={panelRef} onClick={(e)=>e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <strong>Zadonetske</strong>
              <button className="btn btn-ghost" onClick={()=>setOpen(false)} aria-label="Close" ref={closeBtnRef}>✕</button>
            </div>
